...
export const metadata: Metadata = {
  title: 'SharkAuth vs Zitadel: Lightweight Open-Source Auth for Agents',
  description: 'Compare SharkAuth vs Zitadel. Single ~29 MB binary with zero dependencies vs. distributed Go architecture requiring CockroachDB. Agent delegation, DPoP, and cascade revocation.',
  alternates: { canonical: 'https://sharkauth.com/vs-zitadel' },
  openGraph: {
    title: 'SharkAuth vs Zitadel: The Lightweight Alternative',
    description: 'One binary. Zero dependencies. Agent-native auth without the database overhead.',
    url: 'https://sharkauth.com/vs-zitadel',
  },
};

export default function VsZitadelPage() {
  return (
      ...
      <main style={{ maxWidth: 960, margin: '0 auto', padding: '160px 24px 100px' }}>
        <header style={{ marginBottom: 80 }}>
          ...
          <h1 style={{ fontSize: 'clamp(36px, 5vw, 64px)', fontWeight: 500, lineHeight: 1.05, letterSpacing: '-0.04em', marginBottom: 24 }}>
            SharkAuth vs <span className="serif" style={{ color: 'var(--muted)' }}>Zitadel.</span>
          </h1>
          <p style={{ fontSize: 20, lineHeight: 1.6, color: 'var(--muted)', maxWidth: 720 }}>
            Zitadel is a capable open-source identity server written in Go. SharkAuth is an
            agent-native identity platform, also written in Go, that fits in a single binary.
            If you want delegation chains, DPoP, and zero database dependencies, this comparison
            is for you.
          </p>
        </header>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface)' }}>
                  ...
                  <th style={{ padding: '16px 20px', textAlign: 'center', fontWeight: 500, color: 'var(--muted)', fontSize: 13, width: 160 }}>Zitadel</th>
                </tr>
              </thead>
              <tbody>
                {[
                  { feat: 'Deployment model', shark: 'Single binary', zt: 'Distributed (needs DB)' },
                  { feat: 'Binary size', shark: '~29 MB', zt: '~100+ MB' },
                  { feat: 'Dependencies', shark: 'Zero', zt: 'CockroachDB or PostgreSQL' },
                  { feat: 'Cold start time', shark: '~38 ms', zt: '~5-10 seconds' },
                  { feat: 'Agent identity', shark: 'First-class primitive', zt: 'Not native' },
                  { feat: 'RFC 8693 Token Exchange', shark: 'Full', zt: 'Not available' },
                  { feat: 'Act chain depth', shark: 'Native (≥ 4)', zt: 'Not available' },
                  { feat: 'DPoP key binding', shark: 'Native', zt: 'Not available' },
                  { feat: 'Cascade revocation', shark: '< 12 ms p99', zt: 'Manual' },
                  { feat: 'Runs on $5 VPS', shark: 'Yes', zt: 'Difficult (needs DB RAM)' },
                  { feat: 'Air-gapped', shark: 'Yes', zt: 'Yes (with DB)' },
                  { feat: 'Multi-tenant orgs', shark: 'No', zt: 'Yes (native)' },
                  { feat: 'Actions / webhooks', shark: 'Yes', zt: 'Yes (Actions)' },
                ].map((row, i) => (
                  <tr key={row.feat} style={{ borderBottom: i < 12 ? '1px solid var(--border)' : 'none' }}>
                    ...
                    <td style={{ padding: '14px 20px', textAlign: 'center', color: 'var(--muted)' }}>{row.zt}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section style={{ marginBottom: 64, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
          <div style={{ padding: '28px 24px', borderRadius: 14, border: '1px solid var(--border)', background: 'var(--surface)' }}>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {['You want a single binary with zero dependencies', 'You deploy to edge or resource-constrained environments', 'You need agent delegation chains and DPoP', 'You want sub-50ms cold starts', 'You prefer SQLite over CockroachDB'].map(item => (
          ...
          <div style={{ padding: '28px 24px', borderRadius: 14, border: '1px solid var(--border)', background: 'var(--surface)' }}>
            <div className="mono" style={{ fontSize: 11, color: 'var(--muted-2)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>Choose Zitadel if...</div>
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {['You need multi-tenant organization support out of the box', 'You want a mature B2B identity platform', 'You have infrastructure for CockroachDB or PostgreSQL', 'You need SAML 2.0 and SCIM support', 'You want a managed cloud option with SLA'].map(item => (
                <li key={item} style={{ fontSize: 14, color: 'var(--fg)', lineHeight: 1.5, paddingLeft: 16, position: 'relative' }}>
                  <span style={{ position: 'absolute', left: 0, color: 'var(--muted-2)' }}>→</span> {item}
                </li>
              ))}
        ...
        <section style={{ marginBottom: 64, padding: '32px 28px', borderRadius: 14, border: '1px solid var(--border)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 32 }}>
            {[
              { num: '~29 MB', label: 'Binary size', ctx: 'Zitadel: 100+ MB with DB' },
              { num: '~38 ms', label: 'Cold start', ctx: 'Zitadel: 5-10 seconds' },
              { num: '0', label: 'External dependencies', ctx: 'Zitadel: CockroachDB/Postgres' },
              { num: '$5', label: 'VPS to run', ctx: 'Zitadel: needs 2GB+ for DB' },
            ].map(stat => (
              <div key={stat.label}>
        ...
        <section style={{ marginBottom: 64 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {[
              {
                q: 'Can SharkAuth replace Zitadel entirely?',
              ...
              {
                q: 'Why is SharkAuth smaller than Zitadel?',
                a: 'SharkAuth embeds SQLite WAL directly in the binary and requires no external database. Zitadel is designed as a distributed system that requires CockroachDB or PostgreSQL, message queues, and multiple services. SharkAuth intentionally trades distributed scale for deployment simplicity.',
              },
              {
                q: 'Does SharkAuth support the same protocols as Zitadel?',
                a: 'SharkAuth supports OAuth 2.1, OIDC, SAML 2.0 (via SP-initiated), and WebAuthn/Passkeys. Zitadel additionally supports SAML IDP, SCIM, LDAP, and extensive organization management. For standard web and API auth, SharkAuth is sufficient. For enterprise directory integration and B2B multi-tenancy, Zitadel is ahead.',
              },
              {
