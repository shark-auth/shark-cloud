...
interface DelegationCanvasProps {
  revoked: string;
  setRevoked: (val: string) => void;
  compact?: boolean;
}

export function DelegationCanvas({ revoked, setRevoked }: DelegationCanvasProps) {
  // revoked: 'none' | 'B' | 'A'

  const Node = ({ x, y, kind, name, sub, kid, dimmed, statusLabel, statusKind }: any) => (
  ...
  const Connector = ({ from, to, label, dimmed, scope }: any) => {
